...
class SupConLoss(nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        ...
        # Convert labels from list (of strings) to a tensor of integers if needed
        if labels is not None:
            if isinstance(labels, list):
                # Map each unique writer ID to a unique integer
                unique_ids = {w: i for i, w in enumerate(set(labels))}
                labels = torch.tensor([unique_ids[w] for w in labels], device=device)
